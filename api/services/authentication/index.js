...
function restToSocketAuth() {
  return hook => {
    ...
    if (socketId && hook.app.io && accessToken) {
      const { connected } = hook.app.io.sockets;
      const userSocket = Object.values(connected).find(socket => socket.client.id === socketId);
      if (userSocket) {
...
export default function authenticationService() {
  ...
  app
    ...
    // if the name differs from your config key you need to pass your config options explicitly
    .configure(oauth2({
      name: 'facebook',
